# -*- coding: cp1250 -*-
# SMS-nadzorni-center-lokalna-testna.pyw
...
################# Tiskanje ####################
        btnTiskanje = wx.Button(self.panel, label='Tiskaj Zgodovino', pos=(420,630), id=200)
        self.naslovTiskanja = wx.StaticText(self.panel, label='Tiskanje zgodovine', pos=(420,550), style=wx.BOLD)
        self.opisTiskanja = wx.StaticText(self.panel, label='Za tiskanje zgodovine sms-ov (za vse ali za posamezen bazen)\nklikni spodnji gumb. Za prikaz za posamezni bazen klikni na\nustrezen gumb v orodni vrstici (zgoraj).', pos=(420,575))
        self.naslovTiskanja.SetFont(wx.Font(14, wx.NORMAL, wx.NORMAL, wx.BOLD))
################# Tiskanje ####################
...
##  Bind Events
        ...
        self.Bind(wx.EVT_BUTTON, lambda evt: self.TiskajZgodovino(evt), id=200)
    ...
    def TiskajZgodovino(self,e):
        TiskajZgodovino.TiskajZgodovino(None,-1)
    ...
    def PreglejBazo(self, e):
        ...
        try:
            self.prviZagon
            poglej_v_bazo="SELECT * FROM  `smsc`.`objekti` WHERE  `sprememba` =1"
        except:
            poglej_v_bazo="SELECT * FROM  `smsc`.`objekti`"
            self.prviZagon=''
        ...
        for vrsta in vrstice:
            ...
            kateriBazen=self.bazeni.index(obdelujemBazen)
            crpalk=self.crpalkNaBazen[kateriBazen]
            sestevekStanj=vrsta[4]+vrsta[5]+vrsta[6]+vrsta[7]
